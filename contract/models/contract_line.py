...
class ContractLine(models.Model):
    ...
    achievement_pre = fields.Float(string='% Achievement', compute="_compute_total_line_achievement")
    invoice_subtotal = fields.Float(string='Invoice Subtotal', compute="_compute_invoice_line_value")
    ...
    def _check_last_date_invoiced(self):
        for rec in self.filtered("last_date_invoiced"):
            print('======== rec.last_date_invoiced ==========',rec.last_date_invoiced)
    ...
    def _compute_invoice_line_value(self):
        for rec in self:
            total = 0.0
            aml = rec.env['account.move.line'].sudo().search([('contract_line_id', '=', rec.id)])
            print('=========== aml ===========', aml)
            if aml:
                for line in aml:
                    print('======== line.move_id ===========', line.move_id)
                    if line.move_id.state == 'posted':
                        print('=========== enter here ==========')
                        print('========== line.price_subtotal ============', line.price_subtotal)
                        total += line.price_subtotal
            rec.invoice_subtotal = total

    def _compute_total_line_achievement(self):
        for rec in self:
            total = 0.0
            if rec.invoice_subtotal and rec.price_subtotal:
                total = (rec.invoice_subtotal / rec.price_subtotal) * 100
            rec.achievement_pre = total
