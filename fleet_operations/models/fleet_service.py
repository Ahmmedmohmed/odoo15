    ...
    def check_name(self):
        for category in self:
            if self.search_count([
                ...
                raise UserError(_('Service Category with '
...
class FleetVehicleLogServices(models.Model):
    ...
    def create(self, vals):
        ...
        if record.service_type_id:
            repair_lines = []
            for repair_type in record.service_type_id.repair_type_ids:
                repair_lines.append((0, 0, {'repair_type_id': repair_type.id}))
            record.repair_line_ids = repair_lines
        return record
    ...
    def action_done(self):
        ...
        for work_order in self:
            ...
            if work_order.odometer == 0:
                raise UserError(_("Please set the current "
        ...
        if self._context.get('active_ids', False):
            for vehicle in vehicle_obj.browse(self._context['active_ids']):
                if vehicle.state == 'write-off':
                    ...
                    raise UserError(_("New work order can only be "
    ...
    fmp_id = fields.Char(string="Vehicle Name", size=64,
    ...
    state = fields.Selection(selection_add=[('draft', 'New'),
...
class WorkOrderPartsHistoryDetails(models.Model):
    ...
    used_qty = fields.Float(string='Encoded Qty',
                            help='The Quantity that is used in in Work order')
    ...
    wo_id = fields.Many2one('fleet.vehicle.log.services', string='Work order',
...
class TripPartsHistoryDetails(models.Model):
    ...
    def _get_available_qty(self):
        for rec in self:
            ...
            if available_qty < 0:
                raise UserError(_('Quantity Available '
...
class StockMove(models.Model):
    ...
    def onchange_move_type(self):
        ...
        if not self.location_id and not self.location_dest_id:
            ...
            self.location_id = source_location and source_location or False     #source_location[1]
            self.location_dest_id = dest_location and dest_location or False    #source_location[1]
        ...
        for rec in self:
            if rec.product_id:
                ...
                if prod.in_active_part:
                    ...
                    raise UserError(_('You can\'t select '
    ...
    def unlink(self):
        ...
        for part in self:
            if part.fleet_service_id.state == 'done':
                raise UserError(_("You can't delete part those already used."))
            if part.is_deliver:
                raise UserError(_("You can't delete part those already used."))
    ...
    def check_name(self):
        for repair in self:
            if self.search_count([
                ...
                raise UserError(_('Repair type with this name already exists!'))
...
class FleetServiceType(models.Model):
    ...
    category = fields.Selection(selection_add=[('contract', 'Contract'),
    ...
    def check_name(self):
        for service in self:
            if self.search_count([
                ...
                raise UserError(_('Service type with this name already exists!'))
