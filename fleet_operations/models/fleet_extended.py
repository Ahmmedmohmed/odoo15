...
class FleetOperations(models.Model):
    ...
    def _compute_repair_order(self):
        for rec in self:
            rec.repair_order = False
            if rec.log_services:
                done_state = rec.log_services.filtered(lambda c: c.state == 'done')
                if done_state:
                    rec.repair_order = done_state[0].repair_order
    ...
    licensing_start_date = fields.Date(string="Licensing Start Date")
    licensing_end_date = fields.Date(string="Licensing End Date")
    purchase_date = fields.Date(string="Purchase Date")



    customs_release = fields.Selection([('yes', 'Yes'),
                                        ('no', 'No')],
                                       string='Customs Release')

    invoice = fields.Selection([('yes', 'Yes'),
                                        ('no', 'No')],
                                       string='Invoice')


    final_sales_contract = fields.Selection([('yes', 'Yes'),
                                         ('no', 'No')],
                                        string='Final Sales Contract')

    consumption_fuel_rate = fields.Float(string="Consumption Rate")
