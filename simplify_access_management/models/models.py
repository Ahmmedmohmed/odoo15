...
class BaseModel(models.AbstractModel):
    ...
    # في Odoo 15 دوال get_views و load_views غير مدعومة بنفس الطريقة، لذلك سنعتمد كلياً على fields_view_get

    # -----------------------------------------------------------------------------------------
    # تفعيل دالة fields_view_get المتوافقة مع Odoo 15 بدلاً من _get_view
    # -----------------------------------------------------------------------------------------
    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if request and hasattr(request, 'httprequest') and request.httprequest.cookies.get('cids'):
            cids = int(request.httprequest.cookies.get('cids').split(',')[0])

        readonly_access_id = access_management_obj.search(
            [('company_ids', 'in', cids), ('active', '=', True), ('user_ids', 'in', self.env.user.id),
             ('readonly', '=', True)])
        ...
        # معالجة إخفاء الأزرار والأكشنز من الـ Toolbar
        if toolbar and 'toolbar' in res:
            ...
            if res['toolbar'].get('action', False):
                action = [rec for rec in res['toolbar']['action'] if rec.get('id', False) not in remove_server_action]
                res['toolbar']['action'] = action

            if res['toolbar'].get('print', False):
                prints = [rec for rec in res['toolbar']['print'] if rec.get('id', False) not in remove_print_action]
                res['toolbar']['print'] = prints

        if view_type == 'form':
            # إخفاء الحقول ذات الارتباطات الخارجية (External Links)
            ...
            if access_fields_recs:
                ...
                # إخفاء الـ Chatter
            if access_management_obj.search([('company_ids', 'in', self.env.company.id), ('active', '=', True),
                                             ('user_ids', 'in', self.env.user.id), ('hide_chatter', '=', True)],
                                            limit=1).id or self.env['remove.action'].search(
                    [('access_management_id.company_ids', 'in', self.env.company.id),
                     ('access_management_id.active', '=', True),
                     ('access_management_id.user_ids', 'in', self.env.user.id), ('model_id.model', '=', res['model']),
                     ('restrict_chatter', '=', True)], limit=1):
                doc = etree.XML(res['arch'])
        ...
        if view_type in ['kanban', 'tree']:
            restrict_import = access_management_obj.search(
                [('company_ids', 'in', self.env.company.id), ('active', '=', True),
                 ('user_ids', 'in', self.env.user.id), ('hide_import', '=', True)], limit=1).id
            if access_model_recs.filtered(lambda x: x.restrict_import) or restrict_import:
                doc = etree.XML(res['arch'])
                doc.attrib.update({'import': 'false'})
                res['arch'] = etree.tostring(doc, encoding='unicode')

            restrict_export = access_management_obj.search(
                [('company_ids', 'in', self.env.company.id), ('active', '=', True),
                 ('user_ids', 'in', self.env.user.id), ('hide_export', '=', True)], limit=1).id
